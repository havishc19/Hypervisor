...
int activeDoms(virConnectPtr conn, struct DomainList *domainList) {
    virDomainPtr *domainArr;
...
void initializeDomainStats(int nPCPU, DomainList* domainList, DomainStats* currDomainStats){
    /*struct virVcpuInfo {
    ...
    for(int i=0;i<domainList->nDomains; i++){
        ...
        int nVCPU = virDomainGetVcpusFlags(domainList->domainArr[i], VIR_DOMAIN_AFFECT_LIVE);
        if(nVCPU == -1){
        ...
        */
        int temp = virDomainGetVcpus(domainList->domainArr[i], vcpuInfo, nVCPU, cpuMaps, cpuMapSize);
        if(temp == -1){
            printf("Error\n");
            exit(EXIT_FAILURE);
        }
        ...
        currDomainStats[i].domain = domainList->domainArr[i];
    }
    return;
}
...
void pinVcpu(double *pcpuUsage, int nPCPU, DomainStats *currDomainStats, DomainStats *prevDomainStats, int nDomains, double timePeriod){
    ...
    printf("Pinning vCPU %d in Domain %s to pCPU %d \n", leastWtvCpuNumber,
           virDomainGetName(leastWtvCpuDomain), minUtilCpu);
...
int main(int argc, char **argv) {
    ...
    if(domainList == NULL || pcpuUsage == NULL){
    ...
    while (1) { 
        activeDoms(conn, domainList);
        int nDomains = domainList->nDomains;
        if(nDomains <= 0){
            break;
        }
        ...
        initializeDomainStats(nPCPU, domainList, currDomainStats);
        printf("currDomainStats nVCPU: %d\n", currDomainStats[0].nVCPU);

        // Check if number of active domains has changed
        if(nDomains != prevNumberDomains){
            printf("Number of Active Domains has Changed! \n");
            ...
            for(int i=0; i<nDomains; i++){
                for(int j=0; j<currDomainStats[i].nVCPU; j++){
                    double tempTime = ((currDomainStats[i].vcpuTimeArr[j] - prevDomainStats[i].vcpuTimeArr[j]) * 100.0)/ (timePeriod);
                    printf("Domain: %s, vCPU: %d, pCPU: %d, Usage: %f \n", virDomainGetName(domainList->domainArr[i]), j, 
